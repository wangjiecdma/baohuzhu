    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if (view != null) {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                datePicker.setCalendarViewShown(false); datePicker.setSpinnersShown(true); //滚轮模式必须使用确定菜单
                ...
                ensureButton.setVisibility(View.VISIBLE); splitLineV.setVisibility(View.VISIBLE);
                //如果只要日历部分，隐藏header
                ViewGroup mContainer = (ViewGroup) datePicker.getChildAt(0);
                View header = mContainer.getChildAt(0);
                header.setVisibility(View.GONE);
            } else {
                //bug4:LOLLIPOP和Marshmallow上，使用spinner模式，然后隐藏滚轮，显示日历(spinner模式下的日历没有头部)，
                // 日历最底部一排日期被截去部分。所以只能使用calender模式，然后手动隐藏header（系统没有提供隐藏header的api）。
                // 如果只要日历部分，隐藏header
                ViewGroup mContainer = (ViewGroup) datePicker.getChildAt(0);
                View header = mContainer.getChildAt(0); header.setVisibility(View.GONE);
                //Marshmallow上底部留白太多，减小间距
                LinearLayout.LayoutParams layoutParams = (LinearLayout.LayoutParams) datePicker.getLayoutParams();
                layoutParams.bottomMargin = 10; datePicker.setLayoutParams(layoutParams);
            }
            initDatePicker();
        }
    }
